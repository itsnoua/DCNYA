...
async function updatePhaseInfo(phaseId) {
    ...
    if (dateEl)   dateEl.innerText  = 'جاري التحميل...';
    ...
    try {
        const meta = await ApiService.fetchPhaseMetadata(phaseId, state.selectedMunicipality, state.selectedStreet);
        if (meta.start_month) {
            const range = `${ARABIC_MONTHS[meta.start_month]} ${meta.start_year} - ${ARABIC_MONTHS[meta.end_month]} ${meta.end_year}`;
            const lastUpdate = `${meta.today_day} ${ARABIC_MONTHS[meta.today_month]} ${meta.today_year}`;
            if (dateEl) dateEl.innerText = range;
            if (updateEl) updateEl.innerText = lastUpdate;
        } else {
            if (dateEl) dateEl.innerText = meta.date_range || 'لا توجد بيانات';
            if (updateEl) updateEl.innerText = meta.last_update || '-';
        }
    } catch (e) {
        console.error('Error fetching phase metadata:', e);
